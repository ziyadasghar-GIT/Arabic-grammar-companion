...
export default function ExampleCard({ example, index }: { example: Example; index: number }) {
  ...
  const sourceClass = example.source === "nahw-mir" 
  ...
  return (
    <motion.div
      ...
      className="bg-white/90 rounded-xl border border-[#c9a96e]/20 p-5 hover:border-[#c9a96e]/40 transition-all"
    >
      <div className="flex items-start justify-between mb-3">
        <div className={`text-3xl arabic-text leading-relaxed ${caseColorMap[example.case]}`} dir="rtl">
          {example.arabic}
        </div>
